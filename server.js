...
// ── Próximos 24h (football fixtures) ─────────────────────────────────────────
let nextFootball = [];
...
// ── pollOdds — cuotas live para partidos de tenis en curso ───────────────────
// Corre cada 15 segundos de forma independiente al poll principal.
// Guarda en oddsCache: match winner odds, set2 odds, current set odds para el fav.
async function pollOdds() {
  if (!TENNIS_KEY) return;
  const live = lastTennis.filter(m => !m.isUp);
  if (!live.length) return;
  // Lanzar en paralelo pero limitar a 5 para no saturar la API
  const batch = live.slice(0, 5);
  await Promise.allSettled(batch.map(async m => {
    try {
      ...
      // AllSports devuelve un objeto { betType: { Home: {bookmaker:odd,...}, Away: {...} } }
      const bets = r.result;
      let oddMatch = null, oddSet2 = null, oddCurrent = null;
      ...
      const favIs = m.o1!=null&&m.o1>=ODD_MIN&&m.o1<=ODD_MAX ? 'Home'
                  : m.o2!=null&&m.o2>=ODD_MIN&&m.o2<=ODD_MAX ? 'Away' : null;
      for (const [bName, bData] of Object.entries(bets)) {
        const n = bName.toLowerCase();
        if (!oddMatch && (n.includes('match winner') || n.includes('home/away') || n.includes('winner'))) {
          const side = favIs === 'Home' ? (bData['Home']||bData['Player 1']||bData['First Player'])
                                        : (bData['Away']||bData['Player 2']||bData['Second Player']);
          oddMatch = medianOdd(side);
        }
        if (!oddSet2 && n.includes('set 2')) {
          const side = favIs === 'Home' ? (bData['Home']||bData['Player 1']) : (bData['Away']||bData['Player 2']);
          oddSet2 = medianOdd(side);
        }
        if (!oddCurrent && n.includes(`set ${m.curSetNum}`)) {
          const side = favIs === 'Home' ? (bData['Home']||bData['Player 1']) : (bData['Away']||bData['Player 2']);
          oddCurrent = medianOdd(side);
        }
      }
      if (oddMatch || oddSet2 || oddCurrent) {
        oddsCache.set(m._key, {
          match:   oddMatch,
          set2:    oddSet2,
          current: oddCurrent,
          fav:     favIs === 'Home' ? m.p1 : m.p2,
          updated: Date.now()
        });
      }
    } catch(e) { /* silencioso — no romper por un partido */ }
  }));
}

// ── fetchFootballNext — próximas 24h football-data.org ───────────────────────
async function fetchFootballNext() {
  ...
  try {
    ...
    // Solo los no terminados, ordenados por hora
    nextFootball = all
      .filter(m => m.status !== 'FINISHED')
      .sort((a,b) => new Date(a.utcDate) - new Date(b.utcDate));
    console.log(`[NEXT24] ${nextFootball.length} partidos próx. 24h`);
  } catch(e) { console.warn('[NEXT24]', e.message); }
}

// ── Bucles independientes de odds y next24h ───────────────────────────────────
function startOddsPoll() {
  const run = async () => {
    try { await pollOdds(); } catch(e) { console.warn('[ODDS POLL]', e.message); }
    setTimeout(run, 15000);
  };
  setTimeout(run, 8000); // arrancar 8s después del boot
}
function startNext24Poll() {
  const run = async () => {
    try { await fetchFootballNext(); } catch(e) { console.warn('[NEXT24 POLL]', e.message); }
    setTimeout(run, 10 * 60 * 1000); // cada 10 min
  };
  run(); // inmediato al arranque
}
...
function checkTennisAlerts(live){
  live.forEach(m=>{
    ...
    simAlerts.unshift({id:kb,type:'tennis_break',match:`${m.p1} vs ${m.p2}`,
      ...
      odds_match: oddsCache.get(m._key)?.match||null,
      odds_set2:  oddsCache.get(m._key)?.set2||null,
      odds_current: oddsCache.get(m._key)?.current||null});
    if(simAlerts.length>500) simAlerts.length=500;
...
const server=http.createServer((req,res)=>{
  ...
  if(path==='/data'&&req.method==='GET'){
    ...
    res.writeHead(200);
    res.end(JSON.stringify({football:lastFootball,tennis:lastTennis,updated:lastUpdate,alerted:[...alerted],simAlerts:simAlerts.slice(0,200),oddStats,catStats,ftStats,odds:Object.fromEntries(oddsCache)}));
    return;
  }
